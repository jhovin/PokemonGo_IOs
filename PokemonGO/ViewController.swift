    ...
    func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
        ...
        if let coord = self.ubicacion.location?.coordinate{
            ...
            if MKMapRectContainsPoint(mapView.visibleMapRect, MKMapPointForCoordinate(coord)){
                if pokemon.atrapado{
                    let alert = UIAlertController(title: "Ya tienes a este pokemon", message: "¿Aún asu desea capturarlo?", preferredStyle: .alert)
                    let si_capturar = UIAlertAction(title: "Sí", style: .default, handler: nil)
                    alert.addAction(si_capturar)
                    let no_capturar = UIAlertAction(title: "No", style: .default, handler: nil)
                    alert.addAction(no_capturar)
